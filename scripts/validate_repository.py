...
for required_text in ["project-facts.json", "browser-local", "no backend", "Decision Surface"]:
...
if "./assets/universal-decision.css" not in index:
    errors.append("site index does not load the Universal Decision Surface stylesheet")
...
for token in ("draftFromInput", "responseFor", "What FDE sees so far", "Possible is not confirmed"):
    if token not in universal_text:
        errors.append(f"Universal Decision Surface is missing required boundary: {token}")
...
if "--surface-glow:" not in universal_css or ".universal-layout" not in universal_css:
    errors.append("Universal Decision Surface stylesheet is incomplete")
...
print("- Universal Response and Decision Surface are part of the public product boundary")
