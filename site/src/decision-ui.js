...
const restoredPolicy = restored.decision ? decisionRecordability(restored.decision) : { recordable: true, boundary: null };
const restorableDecision = restoredPolicy.recordable ? restored.decision : null;
const restoredHistory = loadDecisionRecordHistory(browserStorage, restorableDecision?.decision_id || '');
const state = {
  step: 0,
  decision: createBlankDecisionCase(),
  source: 'blank',
  pendingDraft: restorableDecision,
  pendingRecord: restorableDecision ? restored.record : null,
  pendingAuthority: restorableDecision ? restored.authority : null,
  authority: createAuthority((restorableDecision ? restored.authority : null) || { owner: restorableDecision?.decision_owner || '' }),
  record: null,
  history: restoredHistory.records,
  historyStatus: restoredHistory.status,
  entryResolved: !restorableDecision,
  maxReached: 0,
  expandAll: false,
  saveStatus: restored.decision && !restoredPolicy.recordable
    ? `${restoredPolicy.boundary?.title || 'Saved decision not restored.'} The saved browser decision remains stored but was not activated.`
    : restored.decision ? 'A browser draft is available.' : restored.status,
  validationIssues: [],
};
...
function frameStep() {
  ...
  return `<div class="stack">
    ...
    <section class="soft-panel stack" data-surface="decision-authority">
      <div><strong>Decision authority</strong><span class="help">Name who owns the decision and your role before comparison.</span></div>
      <div class="grid-2">
    ...
    <details class="soft-panel"><summary><strong>Add context</strong><span class="help">Short name, timing, urgency, and reversibility</span></summary><div class="grid-2 decision-section-body">
...
function mapStep() {
  ...
  const semanticCriteria = item.schema_version === '0.3.0' ? `<section class="stack" data-surface="decision-semantics-criteria">
<div>
<span class="eyebrow">${isSeerProfile(semantics.mode) ? 'Sustainability reminder · SEER' : 'Assurance posture'}</span>
<h3>What must remain true?</h3>
<p class="muted">Review one dimension at a time. Evidence and outcome stay separate; required criteria affect posture, not the comparison.</p>
</div>
<div class="dimension-overview">${dimensions.map((dimension, dimensionIndex) => { const criteria = semantics.criteria.map((criterion, index) => ({ criterion, index })).filter((entry) => entry.criterion.dimension === dimension); const stateLabel = dimension === 'general' ? (criteria.length ? 'Review criteria' : 'Not assessed') : profileSummaries.get(dimension)?.state || 'Not assessed'; const requiredCount = criteria.filter((entry) => entry.criterion.must_be_true).length; return `<details class="panel dimension-card" ${dimensionIndex === 0 ? 'open' : ''} data-dimension="${dimension}">
<summary>
<span>
<strong>${escapeHtml(dimension[0].toUpperCase() + dimension.slice(1))}</strong>
<span class="help">${escapeHtml(dimensionPrompts[dimension])}</span>
</span>
<span class="dimension-status">
<strong>${escapeHtml(stateLabel)}</strong>
<span class="help">${criteria.length} ${criteria.length === 1 ? 'criterion' : 'criteria'}${requiredCount ? ` · ${requiredCount} required` : ''}</span>
<span class="review-label">Review</span>
</span>
</summary>
<div class="dimension-detail stack">${criteria.map(({ criterion, index }) => `<article class="semantic-criterion stack" data-semantic-criterion="${index}">
<div class="grid-2">${field('What matters?', `semantic-label-${index}`, criterion.label)}${field('What must be true?', `semantic-requirement-${index}`, criterion.requirement)}<label class="field">What do we know?<select id="semantic-evidence-${index}">${EVIDENCE_STATES.map((value) => `<option value="${value}" ${criterion.evidence_state === value ? 'selected' : ''}>${escapeHtml(value)}</option>`).join('')}</select>
</label>
<label class="field">Does it meet the requirement?<select id="semantic-outcome-${index}">${CRITERION_OUTCOMES.map((value) => `<option value="${value}" ${criterion.outcome === value ? 'selected' : ''}>${escapeHtml(value)}</option>`).join('')}</select>
</label>${field('What evidence would resolve this?', `semantic-evidence-need-${index}`, criterion.evidence_need || '')}<label class="field required-control">
<span>
<input id="semantic-required-${index}" type="checkbox" ${criterion.must_be_true ? 'checked' : ''}> Required to move forward</span>
</label>
</div>
</article>`).join('')}</div>
</details>`; }).join('')}</div>
...
<div class="actions">
<strong>What success looks like <span class="method-word">(M · measures)</span>
</strong>${badge(`${item.objectives.length}`)}</div>
<p class="help">Use the normalized 0–100 decision-model scale. These values are not probabilities or native scientific or commercial measurements.</p>${item.objectives.map((objective, index) => `<div class="map-item grid-2">${field(`Goal ${index + 1} name`, `objective-label-${index}`, objective.label, 'text', 'Use a meaningful outcome name.', REQUIREMENT_CLASS.COMPARE)}<label class="field" for="objective-threshold-${index}">Good-enough line<span class="requirement">* ${REQUIREMENT_CLASS.COMPARE}</span>
<input id="objective-threshold-${index}" data-objective-threshold="${index}" type="number" min="0" max="100" step="1" value="${objective.threshold ?? ''}" required aria-describedby="objective-threshold-${index}-help">
<span id="objective-threshold-${index}-help" class="help">0–100 normalized scale. ${objective.direction === 'at-most' ? 'Lower values are better.' : 'Higher values are better.'}</span>
</label>
<label class="field">Direction<select data-objective-direction="${index}">
<option value="at-least" ${objective.direction === 'at-least' ? 'selected' : ''}>Meet or exceed</option>
<option value="at-most" ${objective.direction === 'at-most' ? 'selected' : ''}>Stay at or below</option>
</select>
<span class="help">Defaulted to meet or exceed.</span>
</label>${textarea('Helpful detail', `objective-description-${index}`, objective.description, 'Optional.')}</div>`).join('')}</section>
    </div></details>
...
<label for="strategy-score-${strategyIndex}-${objectiveIndex}">${escapeHtml(objective.label || `Goal ${objectiveIndex + 1}`)}<span class="requirement">* ${REQUIREMENT_CLASS.COMPARE}</span>
<input id="strategy-score-${strategyIndex}-${objectiveIndex}" data-strategy-score="${strategyIndex}:${objective.objective_id}" type="number" min="0" max="100" value="${strategy.baseline[objective.objective_id] ?? ''}" required aria-describedby="strategy-score-${strategyIndex}-${objectiveIndex}-help">
</label>
<span id="strategy-score-${strategyIndex}-${objectiveIndex}-help" class="help">Normalized 0–100 decision-model value; not a probability or native measurement. Use only the precision your evidence supports.</span>${state.decision.schema_version === '0.3.0' ? `<details class="score-rationale">
<summary>Why this score?</summary>
<div class="stack">
<label class="field">Basis<select data-score-basis="${strategyIndex}:${objective.objective_id}">
<option value="analyst-judgment" ${trace?.basis !== 'declared-rubric' && trace?.basis !== 'other' ? 'selected' : ''}>Analyst judgment</option>
<option value="declared-rubric" ${trace?.basis === 'declared-rubric' ? 'selected' : ''}>Declared rubric</option>
<option value="other" ${trace?.basis === 'other' ? 'selected' : ''}>Other stated basis</option>
</select>
</label>
<label class="field">Rationale<textarea data-score-rationale="${strategyIndex}:${objective.objective_id}" placeholder="No rationale recorded.">${escapeHtml(trace?.rationale || '')}</textarea>
</label>
</div>
</details>` : ''}</div>`;
  };
...
function scenariosStep() {
  ...
  return `<div class="stack">
    ...
    ${state.decision.scenarios.map((scenario, scenarioIndex) => { const modifiers = strategies.flatMap((strategy) => objectives.map((objective) => scenario.strategy_modifiers?.[strategy.strategy_id]?.[objective.objective_id])); const noChange = modifiers.length > 0 && modifiers.every((value) => value === 0); return `<article class="panel stack">
      ...
      <details>
<summary>
<strong>Advanced / Inspect: changes by choice and goal</strong>
</summary>
<div class="stack" style="margin-top:1rem">
<p class="help">Enter how much this future raises or lowers each choice's normalized value (−100 to +100). Zero means explicitly no modeled change; blank means Not Assessed.</p>${strategies.map((strategy, strategyIndex) => `<section class="soft-panel stack">
<strong>${escapeHtml(strategy.label || `Choice ${strategyIndex + 1}`)}</strong>
<div class="score-grid">${objectives.map((objective, objectiveIndex) => `<label class="score-field" for="scenario-modifier-${scenarioIndex}-${strategyIndex}-${objectiveIndex}">${escapeHtml(objective.label || `Goal ${objectiveIndex + 1}`)} change<span class="requirement">* ${REQUIREMENT_CLASS.COMPARE}</span>
<input id="scenario-modifier-${scenarioIndex}-${strategyIndex}-${objectiveIndex}" data-scenario-strategy-modifier="${scenarioIndex}:${strategy.strategy_id}:${objective.objective_id}" type="number" min="-100" max="100" value="${scenario.strategy_modifiers?.[strategy.strategy_id]?.[objective.objective_id] ?? ''}" required>
<span class="help">Normalized change, not a probability.</span>
</label>`).join('')}</div>
</section>`).join('')}<details class="technical-inspect">
<summary>Inspect bounded future-state mapping</summary>
...
function resultsStep() {
  ...
  const semanticSummary = state.decision.schema_version === '0.3.0' && (isSeerProfile(semantics.mode) || semantics.posture_enabled) ? `<section class="panel stack signature-synthesis" data-surface="decision-posture">
<span class="eyebrow">Brief · Decision signature</span>
<div class="signature-grid">
<div>
<span class="help">Assurance posture</span>
<strong class="posture-value">${escapeHtml(synthesis.posture || 'Inactive')}</strong>
</div>
<div>
<span class="help">Controlling issue</span>
<strong>${escapeHtml(synthesis.controlling_issue)}</strong>
</div>
<div>
<span class="help">Leading tested choice</span>
<strong>${escapeHtml(synthesis.strongest_alternative?.label || 'No unique leader')}</strong>
</div>
<div>
<span class="help">Recorded human decision</span>
<strong>${escapeHtml(synthesis.recorded_human_decision?.label || 'Not recorded')}</strong>
</div>
</div>${synthesis.sustainability_profile.length ? `<div class="grid-4 sustainability-profile-summary">${synthesis.sustainability_profile.map((item) => `<div>
<span class="help">${escapeHtml(item.dimension[0].toUpperCase() + item.dimension.slice(1))}</span>
<strong>${escapeHtml(item.state)}</strong>
</div>`).join('')}</div>` : ''}${synthesis.changes.length ? `<div>
<strong>What would change this decision?</strong>
<ul>${synthesis.changes.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
</div>` : ''}${synthesis.next_evidence ? `<p>
<strong>Most decision-relevant next evidence:</strong> ${escapeHtml(synthesis.next_evidence.evidence_need)}</p>` : ''}<p class="help">The posture, leading tested choice, and recorded human decision are separate. Assurance posture is software decision support—not approval, authorization, certification, qualification, consent, or investment approval.</p>
</section>` : '';
  ...
  const resultFirstSummary = `<section class="decision-value-summary panel stack" data-surface="result-first">
<span class="eyebrow">Decision value</span>
<div class="decision-value-grid">
<div>
<span class="help">What held up</span>
<strong>${escapeHtml(comparisonOutcome)}</strong>
</div>
<div>
<span class="help">Why</span>
<strong>${escapeHtml(comparisonWhy)}</strong>
</div>
</div>
<div>
<h3>What could change it</h3>${decisionValueChanges.length ? `<ul>${decisionValueChanges.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>` : '<p class="muted">No tested vulnerability or explicit change condition is currently surfaced for the comparison target.</p>'}</div>
<div>
<h3>Next Proof</h3>${proofPresentation.items.length ? `<ul>${proofPresentation.items.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>` : `<p class="muted">${escapeHtml(proofPresentation.emptyMessage)}</p>`}</div>
<div>
<h3>Next action</h3>
<p>${escapeHtml(proofPresentation.nextAction)}</p>
</div>
<div class="callout">
<strong>You decide.</strong>
<p class="muted">The comparison informs. A person decides.</p>
</div>
</section>`;
  ...
  return `<div class="stack">
    ...
    <div class="grid-3">${summaries.map((summary) => `<article class="metric-card-like ${summary.strategy_id === candidate?.strategy_id ? `candidate ${summary.critical_failure_scenario_count > 0 ? 'has-critical-gaps' : ''}` : ''}">
<span class="help">${!summary.analysis_valid ? 'Invalid data · not ranked' : summary.strategy_id === candidate?.strategy_id ? (summary.critical_failure_scenario_count > 0 ? 'Strongest alignment in this comparison · goals to strengthen' : 'Strongest alignment in this comparison') : 'Strategy'}</span>
<strong>${escapeHtml(summary.label)}</strong>
<div class="big-number">${summary.analysis_valid ? formatPercent(summary.overall_pass_rate) : '—'}</div>
<span class="help">Share of selected goals reached across tested futures</span>
<div class="result-line">
<span>Most demanding tested future</span>
<strong>${summary.analysis_valid ? formatPercent(summary.worst_case_pass_rate) : '—'}</strong>
</div>
<div class="result-line">
<span>Tested futures with goals to strengthen</span>
<strong>${summary.critical_failure_scenario_count}</strong>
</div>
<div class="result-line">
<span>Outcomes needing attention</span>
<strong>${summary.invalid_outcome_count}</strong>
</div>
</article>`).join('')}</div>
    <details class="matrix-details soft-panel">
<summary>
<strong>See the detailed calculation table</strong>
<span class="help">Detailed values remain available after the plain-language summary.</span>
</summary>
<div class="table-wrap" style="margin-top:1rem">
<table>
<caption>How every choice performed across the tested futures and goals</caption>
<thead>
<tr>
<th scope="col">Strategy</th>
<th scope="col">Scenario</th>${state.decision.objectives.map((objective) => `<th scope="col">${escapeHtml(objective.label)}<br>
<span class="help">Threshold ${objective.threshold}${objective.critical ? ' · critical' : ''}</span>
</th>`).join('')}</tr>
</thead>
<tbody>${state.decision.strategies.flatMap((strategy) => state.decision.scenarios.map((scenario) => `<tr>
<th scope="row">${escapeHtml(strategy.label)}</th>
<td>${escapeHtml(scenario.label)}</td>${state.decision.objectives.map((objective) => { const row = matrix.find((entry) => entry.strategy_id === strategy.strategy_id && entry.scenario_id === scenario.scenario_id && entry.objective_id === objective.objective_id); return `<td>
<strong>${row?.value ?? '—'}</strong>
<br>${outcomeBadge(row, objective)}</td>`; }).join('')}</tr>`)).join('')}</tbody>
</table>
</div>
</details>
</div>
...
<div>
<span class="eyebrow">Conditions to watch and strengthen</span>
<h3>${escapeHtml(state.decision.strategies.find((item) => item.strategy_id === selectedStrategyId)?.label || '')}</h3>
</div>
<p>The tested conditions where the ${vulnerabilitySubject} may need more support to reach a good-enough line.</p>
</div>${vulnerabilities.map((scenario) => `<div class="vulnerability ${scenario.vulnerable ? 'is-vulnerable' : 'is-resilient'}">
<div>
<strong>${escapeHtml(scenario.label)}</strong>
<p class="help">${escapeHtml(scenario.description)}</p>
</div>
<div>${scenario.invalids.length ? badge('Outcome needs attention · comparison paused', 'assumed') : scenario.failures.length ? scenario.failures.map((failure) => badge(failureLabel(failure), failure.critical ? 'interpreted' : 'assumed')).join('') : badge('All selected goals reached', 'measured')}</div>
</div>`).join('')}</section>
...
function workingDecisionBriefText() {
  ...
  const provenanceCounts = { 'analyst-judgment': 0, 'declared-rubric': 0, other: 0, undocumented: 0 };
  for (const strategy of decision.strategies) {
    for (const objective of decision.objectives) {
      const trace = strategy.score_rationales?.[objective.objective_id];
      if (!trace?.rationale?.trim()) provenanceCounts.undocumented += 1;
      else if (trace.basis === 'declared-rubric') provenanceCounts['declared-rubric'] += 1;
      else if (trace.basis === 'other') provenanceCounts.other += 1;
      else provenanceCounts['analyst-judgment'] += 1;
    }
  }
  const scoreProvenance = [
    `Analyst judgment: ${provenanceCounts['analyst-judgment']}`,
    `Declared rubric: ${provenanceCounts['declared-rubric']}`,
    `Other stated basis: ${provenanceCounts.other}`,
    `Not documented: ${provenanceCounts.undocumented}`,
  ];
...
function decisionBriefStep() {
  ...
  const historyPanel = state.history.length
    ? `<details class="soft-panel receipt-history" data-surface="receipt-history"><summary><strong>Local Decision Receipt Archive (${state.history.length})</strong><span class="help">Prior verified receipts preserved in this browser; this is not a cryptographic continuity chain</span></summary><div class="stack decision-section-body">${state.history.map((record, index) => `<div class="history-row"><div><strong>${escapeHtml(record.recorded_at)}</strong><span class="help">${escapeHtml(record.receipt_sha256.slice(0, 16))}…</span></div><button type="button" data-history-download="${index}">Download prior Receipt</button></div>`).join('')}</div></details>`
    : '';
  ...
  const semanticControls = decision.schema_version === '0.3.0' ? `<section class="soft-panel stack" data-surface="semantic-controls">
<div>
<span class="eyebrow">Human-declared proceed conditions</span>
<h3>Assurance posture controls</h3>
</div>
<label class="field">
<span>
<input id="posture-enabled" type="checkbox" ${semantics.posture_enabled ? 'checked' : ''}> Show assurance posture</span>
</label>
<label class="field">Have proceed conditions been reviewed?<select id="proceed-conditions-state">
<option value="unreviewed" ${semantics.proceed_conditions_state === 'unreviewed' ? 'selected' : ''}>Not reviewed</option>
<option value="declared" ${semantics.proceed_conditions_state === 'declared' ? 'selected' : ''}>Required criteria declared</option>
<option value="none-required" ${semantics.proceed_conditions_state === 'none-required' ? 'selected' : ''}>None required — deliberately confirmed</option>
</select>
</label>
<div class="grid-2">${field('Required condition or safeguard', 'semantic-condition-statement', semantics.conditions[0]?.statement || '')}<label class="field">This condition applies to<select id="semantic-condition-target">
<option value="" ${visibleConditionTarget ? '' : 'selected'}>Whole decision (not remediation)</option>${semantics.criteria.map((criterion) => `<option value="${escapeHtml(criterion.criterion_id)}" ${visibleConditionTarget === criterion.criterion_id ? 'selected' : ''}>${escapeHtml(criterion.dimension[0].toUpperCase() + criterion.dimension.slice(1))}: ${escapeHtml(criterion.label || criterion.criterion_id)}</option>`).join('')}</select>
<span class="help">Only an explicitly targeted open condition can remediate that criterion.</span>
</label>
<label class="field">Condition state<select id="semantic-condition-state">
<option value="open" ${semantics.conditions[0]?.state !== 'satisfied' ? 'selected' : ''}>Open</option>
<option value="satisfied" ${semantics.conditions[0]?.state === 'satisfied' ? 'selected' : ''}>Satisfied</option>
</select>
</label>${field('Monitoring obligation', 'semantic-monitoring-observable', semantics.monitoring[0]?.observable || '')}${field('Reassessment', 'semantic-reassessment', semantics.reassessment || '')}</div>
<p class="help">This compact view edits the first condition and monitoring record only; additional imported records remain preserved.</p>
<label class="field">Cautious human posture override<select id="posture-override">
<option value="">No override</option>${['ADVANCE WITH CONDITIONS','REWORK','HOLD','STOP'].map((value) => `<option value="${value}" ${semantics.posture_override === value ? 'selected' : ''}>${value}</option>`).join('')}</select>
</label>${textarea('Why use a more cautious posture?', 'posture-override-reason', semantics.posture_override_reason || '')}<p>
<strong>Current Assurance posture:</strong> ${escapeHtml(posture.posture || 'Inactive')}</p>
<p class="help">These controls do not choose a strategy. Your final decision remains below.</p>
</section>` : '';
  return `<div class="stack">
    ...
    <div class="self-service-panel decision-output" data-recorded="${lifecycle.key === 'recorded'}" data-surface="decision-output">
<div class="record-state ${lifecycle.key}" aria-live="polite">
<strong>${escapeHtml(lifecycle.label)}</strong>${lifecycle.key === 'changed' ? `<p>The working decision changed after ${escapeHtml(state.record.recorded_at)}. Recorded outputs still use the prior recorded version until you review and record again.</p>` : lifecycle.key === 'recorded' ? `<p>Recorded ${escapeHtml(state.record.recorded_at)}. Recording does not mean approval or authorization.</p>` : '<p>The comparison informs. A person decides.</p>'}</div>
<div class="record-prompt actions">${permissions.mayRecordDecision ? `<button id="record-decision" class="primary" type="button">${escapeHtml(lifecycle.key === 'changed' ? 'Review and Record Again' : 'Record human decision')} →</button>` : '<span class="help">Prepare this analysis for the accountable decision owner. Recording is unavailable in advisor mode.</span>'}</div>${recordedOutput}</div>
...
export function buildDecisionHtml(record) {
  ...
  const semantics = semanticView(decision);
  const posture = decisionPosture(decision);
  const semanticSection = decision.schema_version === '0.3.0' && (isSeerProfile(semantics.mode) || semantics.posture_enabled) ? `<section>
<h2>Decision signature</h2>
<p>
<strong>Assurance posture:</strong> ${escapeHtml(synthesis.posture || 'Inactive')}</p>
<p>
<strong>Controlling issue:</strong> ${escapeHtml(synthesis.controlling_issue)}</p>
<p>
<strong>Leading tested choice:</strong> ${escapeHtml(synthesis.strongest_alternative?.label || 'No unique leader')}</p>
<p>
<strong>Recorded human decision:</strong> ${escapeHtml(synthesis.recorded_human_decision?.label || 'Not recorded')}</p>${synthesis.sustainability_profile.length ? `<div class="grid">${synthesis.sustainability_profile.map((item) => `<div>
<h3>${escapeHtml(item.dimension[0].toUpperCase() + item.dimension.slice(1))}</h3>
<p>${escapeHtml(item.state)}</p>
</div>`).join('')}</div>` : ''}${synthesis.changes.length ? `<h3>What would change this decision?</h3>${list(synthesis.changes)}` : ''}${synthesis.next_evidence ? `<p>
<strong>Most decision-relevant next evidence:</strong> ${escapeHtml(synthesis.next_evidence.evidence_need)}</p>` : ''}<p>
<em>The posture, leading tested choice, and recorded human decision are separate. Assurance posture is software decision support, not approval or authorization.</em>
</p>
</section>` : '';
  return `
<!doctype html><html lang="en">
<head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(decision.title)}</title><style>body{font-family:system-ui,sans-serif;max-width:880px;margin:40px auto;padding:0 20px;line-height:1.55;color:#172033}section{margin:28px 0;padding-top:12px;border-top:1px solid #d7dce5}table{width:100%;border-collapse:collapse}caption{text-align:left;font-weight:700;margin-bottom:8px}th,td{text-align:left;padding:8px;border-bottom:1px solid #d7dce5}.tag{display:inline-block;border:1px solid #aeb7c7;border-radius:999px;padding:3px 9px;margin:2px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:24px}@media(max-width:650px){.grid{grid-template-columns:1fr}}@media print{body{max-width:none;margin:0;padding:0;color:#000}section{break-inside:avoid}.no-print{display:none}}</style></head>
<body>
<h1>${escapeHtml(decision.title)}</h1><p>${escapeHtml(decision.question)}</p><p><span class="tag">${escapeHtml(decision.profile)}</span><span class="tag">Human-governed</span><span class="tag">${decision.provenance.probability_model_used ? 'Probability model disclosed' : 'No probability model'}</span></p><section>
<h2>Decision and accountable authority</h2><p><strong>Decision owner:</strong> ${escapeHtml(record.format_version === '2' ? record.authority.owner : decision.decision_owner)}</p><p><strong>Authority:</strong> ${escapeHtml(record.format_version === '2' ? AUTHORITY_LABELS[record.authority.role] : 'Legacy recorded decision')}</p>${record.format_version === '2' && record.authority.basis ? `<p><strong>Authority basis:</strong> ${escapeHtml(record.authority.basis)}</p>` : ''}</section>
<section>
<h2>Recorded decision</h2><p><strong>Leading tested choice:</strong> ${escapeHtml(machineCandidateLabel)}${candidate?.critical_failure_scenario_count ? ` <em>(leading tested choice has critical gaps in ${candidate.critical_failure_scenario_count} included future${candidate.critical_failure_scenario_count === 1 ? '' : 's'})</em>` : ''}</p><p><strong>Recorded human decision:</strong> ${escapeHtml(selected?.label || 'Not recorded')}</p><p>${escapeHtml(decision.human_decision.rationale)}</p><p><strong>Next action:</strong> ${escapeHtml(decision.human_decision.next_action)}</p><p><em>Recording documents the human decision; it is not approval, authorization, certification, qualification, consent, or investment authority.</em></p></section>
${semanticSection}<section>
<h2>Evidence gate</h2>${record.format_version === '2' ? `<p><strong>Disposition:</strong> ${escapeHtml(record.evidence_disposition.state)}</p>${record.evidence_disposition.unresolved_evidence.length ? `<p><strong>Unresolved evidence:</strong> ${escapeHtml(record.evidence_disposition.unresolved_evidence.join(', '))}</p><p>${escapeHtml(record.evidence_disposition.rationale)}</p>` : ''}` : '<p>Legacy record; receipt-level evidence disposition was not recorded.</p>'}</section>
<section>
<h2>What FDE did not establish</h2><ul><li>FDE does not independently verify source claims unless separately provided through governed evidence.</li><li>Normalized comparison inputs are not probabilities or native scientific or commercial measurements.</li><li>A recorded human decision is not organizational approval or execution authorization.</li><li>The receipt does not prove that the decision was correct.</li></ul></section>
<section>
<h2>Evidence boundary</h2><div class="grid"><div><h3>Known</h3>${list(decision.evidence_summary.known)}</div><div><h3>Assumed</h3>${list(decision.evidence_summary.assumed)}</div><div><h3>Unknown</h3>${list(decision.evidence_summary.unknown)}</div><div><h3>Recorded human choice vulnerabilities</h3>${vulnerabilities.length ? `<ul>${vulnerabilities.map((item) => `<li><strong>${escapeHtml(item.label)}:</strong> ${item.failures.map((failure) => escapeHtml(decision.objectives.find((objective) => objective.objective_id === failure.objective_id)?.label || failure.objective_id)).join(', ')}</li>`).join('')}</ul>` : '<p>No declared threshold failures for the recorded human choice in the included futures.</p>'}</div></div></section>
<section>
<h2>Choice comparison</h2><p>Normalized values use a 0–100 decision-model scale. They are not probabilities or native scientific or commercial measurements.</p><table>
<caption>Goal performance by choice</caption><thead>
<tr><th scope="col">Choice</th><th scope="col">Overall pass</th><th scope="col">Most demanding future</th><th scope="col">Critical-failure futures</th><th scope="col">Critical misses</th></tr></thead>
<tbody>
${summaries.map((item) => `<tr><th scope="row">${escapeHtml(item.label)}</th><td>${formatPercent(item.overall_pass_rate)}</td><td>${formatPercent(item.worst_case_pass_rate)}</td><td>${item.critical_failure_scenario_count}</td><td>${item.critical_failure_count}</td></tr>`).join('')}</tbody>
</table>
</section>
${decision.adaptive_pathway.act_now.length || decision.adaptive_pathway.monitor.length || decision.adaptive_pathway.triggers.length || decision.adaptive_pathway.contingencies.length || decision.adaptive_pathway.reassessment ? `<section>
<h2>Optional plan for change</h2><div class="grid"><div><h3>Act now</h3>${list(decision.adaptive_pathway.act_now)}</div><div><h3>Monitor</h3>${list(decision.adaptive_pathway.monitor)}</div><div><h3>Triggers</h3>${list(decision.adaptive_pathway.triggers)}</div><div><h3>Contingencies</h3>${list(decision.adaptive_pathway.contingencies)}</div></div><p><strong>Reassessment:</strong> ${escapeHtml(decision.adaptive_pathway.reassessment)}</p></section>
` : ''}${record.format_version === '2' ? `<section>
<h2>Human attestation</h2><p>${escapeHtml(record.attestation.statement)}</p><p><strong>Attested by:</strong> ${escapeHtml(record.attestation.attested_by)} · ${escapeHtml(record.attestation.attested_role)}</p><p><strong>Attested at:</strong> ${escapeHtml(record.attestation.attested_at)}</p></section>
<section>
<h2>Integrity</h2><p><strong>FDE version:</strong> ${escapeHtml(record.producer.application_version)}</p><p><strong>Decision schema:</strong> ${escapeHtml(record.producer.decision_schema_version)}</p><p><strong>Semantic schema:</strong> ${escapeHtml(record.producer.semantic_schema_version)}</p><p><strong>Decision content SHA-256:</strong> <code>${escapeHtml(record.decision_content_sha256)}</code></p><p><strong>Receipt SHA-256:</strong> <code>${escapeHtml(record.receipt_sha256)}</code></p></section>
` : `<section>
<h2>Integrity</h2><p><strong>Legacy integrity receipt:</strong> FNV-1a change detection only; not a cryptographic receipt.</p></section>
`}<p>Generated by Frontier Decision Engine v${APPLICATION_VERSION}.</p></body>
</html>
`;
}
...
async function openDecisionFile(file, main, input) {
  ...
  const recordability = decisionRecordability(parsed.decision);
  if (!recordability.recordable) {
    if (message) message.textContent = `${recordability.boundary.title} ${recordability.boundary.body}`;
    message?.focus();
    input.value = '';
    return;
  }
...
function bindEvents(root) {
  ...
  root.querySelector('#record-decision')?.addEventListener('click', () => {
    syncStep();
    const recordability = decisionRecordability(state.decision);
    if (!recordability.recordable) {
      const message = `${recordability.boundary.title} ${recordability.boundary.body}`;
      state.validationIssues = [{ stage: 5, fieldId: 'recordability-boundary', message }];
