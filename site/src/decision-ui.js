...
import { SEER_DIMENSIONS, SEER_DIMENSION_PROMPTS, SEER_PROFILE_ID, SEER_PROFILE_LABEL, isSeerProfile } from './lib/profiles/seer.js';
...
function frameStep() {
  ...
  return `<div class="stack">
    ...
    <details class="soft-panel"><summary><strong>Choose a decision approach</strong><span class="help">Optional method and posture controls</span></summary><div class="stack decision-section-body"><label class="field">Assurance profile<select id="decision-semantic-mode"><option value="general" ${semantics.mode === 'general' ? 'selected' : ''}>General assurance</option><option value="${SEER_PROFILE_ID}" ${isSeerProfile(semantics.mode) ? 'selected' : ''}>${SEER_PROFILE_LABEL}</option></select><span class="help">SEER adds People, Planet, Profits, and Product as independent sustainability criteria without changing human decision authority.</span></label><label class="field"><span><input id="enable-decision-posture" type="checkbox" ${semantics.posture_enabled ? 'checked' : ''}> Use assurance posture</span><span class="help">Assurance posture is available in General mode and enabled by the SEER sustainability profile. It never changes the comparison.</span></label></div></details>
  </div>`;
}
