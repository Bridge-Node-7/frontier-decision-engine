...
export function renderUniversalDecisionExperience(root) {
  ...
  function paint(focusId = '') {
    ...
    const nextText = state.next || (enough ? 'Enough structure is visible to build the formal FDE comparison.' : 'You can stop here, or add only the detail that would help you most.');
    root.innerHTML = `<section class="universal-hero" aria-labelledby="universal-title"><span class="eyebrow">Frontier Decision Engine</span><h1 id="universal-title">Bring the whole mess.</h1><p class="universal-subtitle">You don’t need to know how to use FDE. Tell it what’s happening, and FDE will give you something useful back.</p></section><div class="universal-layout"><section class="universal-entry" aria-labelledby="universal-response-title"><div class="universal-response" role="status" aria-live="polite"><span class="universal-response-kicker">FDE</span><h2 id="universal-response-title" tabindex="-1">${escapeHtml(response.title)}</h2><p>${escapeHtml(response.body)}</p></div><label for="universal-input">What’s on your mind?</label><textarea id="universal-input" maxlength="${RESCUE_MAX_INPUT_CHARS}" rows="10" aria-describedby="universal-help" placeholder="Type anything… a problem, question, worry, idea, decision, or a complete mess.">${escapeHtml(state.startingPoint)}</textarea><p id="universal-help" class="help">Anything is okay. Your text stays in this browser and is treated as input—not as verified evidence.</p><div class="universal-actions"><button id="universal-analyze" class="primary" type="button">Show me what FDE sees →</button><button id="universal-clear" class="quiet" type="button">Start over</button></div><p id="universal-status" class="universal-status" role="status" aria-live="polite"></p></section><aside class="universal-surface" aria-labelledby="surface-title"><div class="universal-surface-head"><div><span class="eyebrow">Live decision surface</span><h2 id="surface-title">What FDE sees so far</h2></div><span class="universal-badge">Draft</span></div>${surfaceCard({ label: 'Possible decision', values: state.possibleDecision ? [state.possibleDecision] : [], empty: 'Not clear yet.' })}${surfaceCard({ label: 'Choices mentioned', values: state.choices, empty: 'No explicit choices yet.' })}${surfaceCard({ label: 'What may matter', values: state.goals, empty: 'Nothing safely identified yet.' })}${surfaceCard({ label: 'What could change the answer', values: state.futures, empty: 'Nothing safely identified yet.' })}<section class="universal-next" aria-labelledby="universal-next-title"><div class="universal-card-label" id="universal-next-title">Most useful next step</div><p>${escapeHtml(nextText)}</p><div class="universal-confirm-row">${enough ? '<button id="universal-confirm" type="button">Build my FDE decision →</button>' : '<a class="button" href="#/rescue">Help me shape the missing pieces →</a>'}<button id="universal-download" class="quiet" type="button">Download this surface</button></div></section><p class="universal-truth"><strong>Possible is not confirmed.</strong> FDE does not turn your words into facts, probabilities, scores, or recommendations without explicit inputs. The comparison informs; a person decides.</p></aside></div>`;
    ...
    root.querySelector('#universal-analyze')?.addEventListener('click', () => {
      ...
      state.next = state.choices.length || state.goals.length || state.futures.length ? 'Review the draft surface. Nothing here is confirmed until you say it is.' : 'You already have a useful starting point. Add detail only when it would help you.';
      saveSession(state); paint('#surface-title');
    });
    ...
    root.querySelector('#universal-confirm')?.addEventListener('click', () => {
      ...
      if (saved?.getItem?.(DECISION_STORAGE_KEY)) { state.next = 'A saved FDE decision already exists. Open the Decision Lab to review it before replacing anything.'; paint('#surface-title'); return; }
